...
- ✅ **已确认（2026-05-15）审批流程**：医生制作完成后 → 医学编辑审核 → [医学编辑修改] → PX运营审核 → 药企审核 → 患者端发布。
- ✅ **已确认（2026-05-15）驳回策略**：所有节点审核不通过均打回至「医学编辑修改」，修改完成后继续流转。
- `cancelled` 状态如何触发和展示。
...
```text
DX 医学审核 → PX 运营审核 → 药企审核
```

Live parity note（2026-05-15）：`/approvals` 的待审列表按诉求分组，展开后显示“项目 · … · 诉求 · …”副标题、`DX 医学审核` 当前节点、SLA；点击“查看 / 处理”打开右侧抽屉，可在链路、历史、通过/不通过、审批意见和提交按钮之间完成单条处理。
...
Live parity note（2026-05-14）：`/distribute/request/REQ-2031` 以 PRJ-1000「赫赛汀 · HER2+ 术后辅助随访计划」为基准，默认 6 篇诉求矩阵、策略分发启用、指定分发关闭；候选医生按互动数 desc 且排除待写 > 0 医生后展示 6 位策略候选。

### 4.5 生产待确认

- ✅ **已确认（2026-05-15）**：分发项目与内容项目是同一实体。系统层级为：租户→项目→诉求→分发。
- ✅ **已确认（2026-05-15）**：分发按诉求维度进行，支持一次分发少量，多次分发完成。
- ✅ **已确认（2026-05-15）**：一篇文章=一个任务（SubTask），含医生ID/主题/形式/病种/药品。主题×形式随机分配，不支持指定。
- ✅ **已确认（2026-05-15）**：互动分=该医生历史内容的点赞+收藏之和。策略分发按互动分desc排序均分，余数补给高互动分者。指定分发选中的医生不再参与策略分发。
- 分发进度是否由节点完成率自动计算。
...
Live parity note（2026-05-15）：`/admin/tenants` 展示 7 个租户、5 个启用、1 个停用、18 个账号；列表提供搜索、状态筛选、病种数量/账号数和启用开关，详情右侧抽屉展示销售经理、成功经理、病种/品牌/区域范围和真实 demo 账号。详情内“邀请账号”打开右侧邀请抽屉并写入 `users`；“新增租户”向导按“基础信息 → 合规可见范围 → 邀请管理员 → 摘要确认”执行，并写入租户、范围与首位管理员。
