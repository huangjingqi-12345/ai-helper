...
- 默认采用科技感强的深色渐变/暗色卡片视觉：以深蓝、蓝紫、墨蓝、暗青、低饱和深灰蓝为主，搭配高亮蓝/青/紫/橙等发光强调色。
- 禁止做成纯黑或全黑背景；页面应有渐变层次、半透明卡片、光效线条、网格/粒子/柔光装饰等层次，整体高级但不压抑。
- 除非用户明确要求浅色、简洁白底或沿用已有浅色模板，否则不要使用大面积白色/米色背景。深色科技感下必须保证文字高对比度和可读性，正文优先使用白色或浅灰，关键数据使用高亮色，不要为了炫酷牺牲阅读。
- 封面必须有主视觉，不能只是标题加背景。
...
- 局部精美化默认向科技感更强的深色渐变/暗色卡片视觉升级：以深蓝、蓝紫、墨蓝、暗青、低饱和深灰蓝为主，搭配高亮蓝/青/紫/橙等发光强调色。禁止纯黑或全黑背景；应通过渐变层次、半透明卡片、光效线条、网格/粒子/柔光装饰营造高级科技感。若原 deck 是浅色，也可以只对目标页做深色卡片/深色主视觉区，但不要破坏整套主题连续性。
- 页面要有清晰层级：主标题、核心结论、主体图表/卡片、辅助说明。
- 合理留白，重要内容不能贴边，核心区建议保留安全边距。
...
export const SHORTCUT_PROMPTS: Record<string, string> = {
  ...
  '/ppt-svg': `请帮我生成一份患教运营汇报 PPT 精美版，基于最近一年数据，用于正式汇报场景，要求视觉完成度更高、页面更精致、层次更清晰；请围绕阅读与互动趋势、内容和项目表现、关键变化、潜在问题、原因判断和下一步行动建议组织 6-8 页；设计上默认采用科技感强的深色渐变/暗色卡片视觉，避免纯黑或全黑背景，封面有清晰汇报主题和主视觉，全篇使用统一色彩、字体层级、卡片风格和图表风格，页面包含清晰主标题、核心结论、数据图表或指标卡片、辅助说明和适度留白，图表、KPI 卡、时间线、矩阵、排行等组件要精细，避免普通表格或文字堆叠；每页文字必须可读，重要结论必须有数据支撑并给出业务含义或行动建议，最终请生成可编辑 PPT。`,
};
