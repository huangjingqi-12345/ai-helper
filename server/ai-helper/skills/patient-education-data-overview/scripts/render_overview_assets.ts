...
function projectTreemap(items: Array<{ label: string; value: number; hint?: string }>): string {
  ...
  const cells = rows.map((x, i) => {
    ...
    return `<div class="tree-cell" style="--tree:${chartColor(i)};flex-basis:${basis.toFixed(1)}%"><span>${esc(short(x.label, 16))}</span><strong>${fmt(x.value)}</strong><small>${pct(share, 0)}${x.hint ? ` · ${esc(x.hint)}` : ''}</small></div>`;
  }).join('');
...
function donutChart(items: Array<{ label: string; value: number }>, title: string, center: string, sub: string): string {
  ...
  const legends = items.slice(0, 4).map((x, i) => `<div class="donut-legend"><i style="background:${chartColor(i)}"></i><span>${esc(short(x.label, 14))}</span><b>${pct(safeDiv(x.value, total), 0)}</b></div>`).join('');
...
function stackedShare(items: Array<{ label: string; value: number }>): string {
  ...
  const legend = items.slice(0, 4).map((x, i) => `<span><i style="background:${chartColor(i)}"></i>${esc(short(x.label, 12))}</span>`).join('');
...
.board{position:relative;border-radius:22px;overflow:hidden;border:1px solid rgba(148,163,184,.20);background:linear-gradient(135deg,rgba(255,255,255,.94),rgba(255,255,255,.78) 46%,rgba(255,255,255,.88)),radial-gradient(circle at 10% 0%,color-mix(in srgb,var(--c1) 12%,transparent),transparent 30%),radial-gradient(circle at 90% 10%,color-mix(in srgb,var(--c3) 10%,transparent),transparent 28%),radial-gradient(circle at 50% 110%,color-mix(in srgb,var(--c5) 10%,transparent),transparent 34%);box-shadow:0 28px 80px rgba(15,23,42,.10);padding:30px}
.board:before{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(255,255,255,.20) 1px,transparent 1px),linear-gradient(0deg,rgba(255,255,255,.18) 1px,transparent 1px);background-size:48px 48px;mask-image:linear-gradient(180deg,rgba(0,0,0,.28),transparent 64%);pointer-events:none}
...
.rank-row{display:grid;grid-template-columns:28px 1fr 88px;gap:12px;align-items:center;margin:12px 0}.rank-index{width:28px;height:28px;border-radius:10px;background:color-mix(in srgb,var(--bar) 10%,white);color:color-mix(in srgb,var(--bar) 78%,var(--ink));font-weight:950;display:grid;place-items:center}.rank-title{font-weight:850;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rank-num{text-align:right;font-weight:950;color:color-mix(in srgb,var(--bar) 78%,var(--ink))}.track{height:11px;border-radius:999px;background:color-mix(in srgb,var(--muted) 15%,transparent);overflow:hidden;margin-top:7px}.fill{height:100%;border-radius:999px;background:linear-gradient(90deg,color-mix(in srgb,var(--bar) 70%,white),color-mix(in srgb,var(--bar) 25%,white))}.hint{font-size:11px;color:var(--muted);margin-top:4px}
...
.treemap{min-height:230px;display:flex;gap:10px;flex-wrap:wrap;align-content:stretch}.tree-cell{min-width:150px;min-height:96px;border-radius:18px;padding:14px;display:grid;align-content:space-between;background:linear-gradient(145deg,color-mix(in srgb,var(--tree) 16%,white),color-mix(in srgb,var(--tree) 34%,white));border:1px solid color-mix(in srgb,var(--tree) 16%,transparent);box-shadow:0 14px 28px color-mix(in srgb,var(--tree) 10%,transparent)}.tree-cell span{font-weight:950;line-height:1.25}.tree-cell strong{font-size:26px;letter-spacing:-.04em;color:color-mix(in srgb,var(--tree) 72%,var(--ink))}.tree-cell small{color:var(--muted);font-weight:800;line-height:1.35}
.content-matrix{min-height:240px;display:grid;grid-template-columns:1fr 1fr;gap:10px}.matrix-cell{border-radius:16px;background:linear-gradient(145deg,color-mix(in srgb,var(--m) 9%,white),rgba(255,255,255,.78));border:1px solid color-mix(in srgb,var(--m) 14%,transparent);padding:12px;display:grid;grid-template-rows:auto 1fr;gap:8px}.matrix-head b{display:block;color:color-mix(in srgb,var(--m) 78%,var(--ink));font-size:14px}.matrix-head span{display:block;margin-top:2px;color:var(--muted);font-size:11px;font-weight:850}.matrix-items{display:grid;gap:6px;align-content:start}.matrix-item{display:grid;grid-template-columns:20px 1fr;grid-template-rows:auto auto;column-gap:7px;align-items:center;border-radius:12px;background:rgba(255,255,255,.62);padding:7px}.matrix-item i{grid-row:1/3;width:20px;height:20px;border-radius:999px;background:color-mix(in srgb,var(--m) 22%,white);color:color-mix(in srgb,var(--m) 78%,var(--ink));display:grid;place-items:center;font-style:normal;font-weight:950;font-size:11px}.matrix-item strong{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.matrix-item span{font-size:10.5px;color:var(--muted);font-weight:800}.matrix-items em{font-style:normal;color:var(--muted);font-weight:850;font-size:12px}
.donut-wrap{display:grid;grid-template-columns:160px 1fr;grid-template-rows:auto 1fr;gap:12px;align-items:center}.donut-title{grid-column:1/-1;font-weight:950;font-size:20px}.donut{width:156px;height:156px;border-radius:50%;display:grid;place-items:center;box-shadow:0 18px 32px rgba(15,23,42,.10)}.donut>div{width:92px;height:92px;border-radius:50%;background:rgba(255,255,255,.94);display:grid;place-items:center;text-align:center;padding:10px}.donut strong{display:block;font-size:24px;line-height:1;color:var(--ink)}.donut span{display:block;margin-top:5px;font-size:11px;color:var(--muted);font-weight:800}.donut-legends{display:grid;gap:8px}.donut-legend{display:grid;grid-template-columns:10px 1fr 42px;gap:8px;align-items:center;font-size:12px;font-weight:850}.donut-legend i{width:10px;height:10px;border-radius:999px}.donut-legend b{text-align:right}
.stacked-bar{height:34px;border-radius:999px;overflow:hidden;display:flex;background:color-mix(in srgb,var(--muted) 14%,transparent);box-shadow:inset 0 0 0 1px rgba(255,255,255,.42)}.stacked-bar i{height:100%}.stacked-legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:14px}.stacked-legend span{font-size:12px;font-weight:850;color:var(--muted)}.stacked-legend i{display:inline-block;width:10px;height:10px;border-radius:999px;margin-right:6px}
