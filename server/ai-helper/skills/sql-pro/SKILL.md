...
This project exposes a controlled read-only execution action for PostgreSQL:
...
```json
{
  ...
  "thought": "why this read-only query is needed"
}
```
...
- Prefer `SELECT` or `WITH ... SELECT` queries.
- `EXPLAIN` is allowed, but `EXPLAIN ANALYZE` is not allowed because it executes the query.
- DDL/DML/admin statements are forbidden, including `INSERT`, `UPDATE`, `DELETE`, `MERGE`, `CREATE`, `ALTER`, `DROP`, `TRUNCATE`, `GRANT`, `REVOKE`, `VACUUM`, `CALL`, and `COPY`.
- Use `max_rows` to keep result sets small; default is 100.
