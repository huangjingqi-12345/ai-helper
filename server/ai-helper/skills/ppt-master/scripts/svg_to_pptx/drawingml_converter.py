...
def _parse_root_viewbox(root: ET.Element) -> tuple[float, float, float, float] | None:
...
def _fit_shapes_xml_to_slide(
    ...
    slide_w = int(target_pixels[0] * EMU_PER_PX)
    slide_h = int(target_pixels[1] * EMU_PER_PX)
    ...
    if min_x >= 0 and min_y >= 0 and max_x <= slide_w and max_y <= slide_h:
        return shapes_xml

    scale = min(slide_w / content_w, slide_h / content_h)
    fitted_w = int(round(content_w * scale))
    fitted_h = int(round(content_h * scale))
    off_x = int(round((slide_w - fitted_w) / 2))
    off_y = int(round((slide_h - fitted_h) / 2))
    group_id = ctx.next_id()
    if verbose:
        print(
            f'  Fitted converted bounds to slide '
            f'({content_w}×{content_h} EMU → {fitted_w}×{fitted_h} EMU)'
        )
    return f'''<p:grpSp>
<p:nvGrpSpPr>
<p:cNvPr id="{group_id}" name="Slide Fit Group {group_id}"/>
<p:cNvGrpSpPr/>
<p:nvPr/>
</p:nvGrpSpPr>
<p:grpSpPr>
...
def convert_svg_to_slide_shapes(
    ...
    Args:
        ...
        target_pixels: Optional slide pixel size. When provided, the root SVG
          viewBox is uniformly scaled and centered into this box so oversized
          SVG canvases do not overflow the PowerPoint slide.
    ...
    scale = 1.0
    if target_pixels:
        view_box = _parse_root_viewbox(root)
        if view_box:
            min_x, min_y, view_w, view_h = view_box
            target_w, target_h = target_pixels
            if view_w > 0 and view_h > 0 and target_w > 0 and target_h > 0:
                scale = min(target_w / view_w, target_h / view_h)
                fitted_w = view_w * scale
                fitted_h = view_h * scale
                translate_x = (target_w - fitted_w) / 2 - min_x * scale
                translate_y = (target_h - fitted_h) / 2 - min_y * scale
                if verbose and abs(scale - 1.0) > 0.0001:
                    print(
                        f'  Scaled root viewBox {view_w:g}×{view_h:g} '
                        f'to fit {target_w}×{target_h} (scale={scale:.4f})'
                    )
    ...
    shapes: list[str] = []
    root_bounds: list[tuple[int, int, int, int]] = []
    converted = 0
    skipped = 0
    ...
    for child in root:
        ...
        if result:
            ...
            if result.bounds_emu:
                root_bounds.append(result.bounds_emu)
            converted += 1
    ...
    shapes_xml = '\n'.join(shapes)
    if target_pixels and root_bounds:
        shapes_xml = _fit_shapes_xml_to_slide(
            shapes_xml,
            root_bounds,
            ctx,
            target_pixels,
