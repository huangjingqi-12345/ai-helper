...
from .keynote_export import export_keynote_pptx
...
''',
    ...
    parser.add_argument(
        '--keynote',
        action='store_true',
        help='Also export a Keynote-friendly picture-only PPTX (macOS qlmanage, not editable)',
    )
    parser.add_argument(
        '--keynote-scale',
        type=int,
        default=4,
        help='Keynote export raster scale (export px = viewBox * scale); default 4',
    )
    parser.add_argument(
        '--keynote-supersample',
        type=int,
        default=2,
        help='Keynote export qlmanage supersample factor; default 2',
    )
    parser.add_argument(
        '--keynote-png-compress',
        type=int,
        default=6,
        help='Keynote export PNG compression 0-9 (lossless); default 6',
    )
    ...
    # --- Keynote-friendly picture-only version (macOS) ---
    if args.keynote:
        keynote_path = native_path.with_name(native_path.stem + "_keynote.pptx")
        try:
            export_keynote_pptx(
                svg_files=native_files if native_files else legacy_files,
                output_path=keynote_path,
                scale=args.keynote_scale,
                supersample=args.keynote_supersample,
                png_compress=args.keynote_png_compress,
            )
            if verbose:
                print(f"[Done] Saved (Keynote): {keynote_path}")
        except Exception as exc:
            success = False
            print(f"[Keynote] Export failed: {exc}", file=sys.stderr)
