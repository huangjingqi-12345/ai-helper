...
def _force_presentation_size_type(presentation_xml: Path, width_emu: int, height_emu: int) -> None:
    """Keep PowerPoint UI aspect ratio in sync with explicit EMU dimensions."""
    tree = ET.parse(presentation_xml)
    root = tree.getroot()
    size = root.find(f'{{{PML_NS}}}sldSz')
    if size is None:
        return
    size.set('cx', str(width_emu))
    size.set('cy', str(height_emu))
    if width_emu * 9 == height_emu * 16:
        size.set('type', 'wide')
    elif width_emu * 3 == height_emu * 4:
        size.set('type', 'screen4x3')
...
def create_pptx_with_native_svg(
    ...
    try:
        ...
        _force_presentation_size_type(extract_dir / 'ppt' / 'presentation.xml', width_emu, height_emu)
                ...
                if use_native_shapes:
                    ...
                    slide_xml, media_files_dict, rel_entries, anim_targets = (
                        convert_svg_to_slide_shapes(
                            ...
                            slide_num=slide_num,
                            verbose=verbose,
                            target_pixels=(pixel_width, pixel_height),
                        )
                    )
