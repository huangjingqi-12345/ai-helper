...
5. Spacing / border radius — Executor adjusts per content density and generated page role
...
4. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of **visual tokens and technical constraints only**: canvas, colors, typography, icon inventory, image file policy, and forbidden SVG features. This file is what the Executor re-reads before every page to prevent visual drift (see [executor-base.md](executor-base.md) §2.1).
   - **Do not lock content in `spec_lock.md`**: no page count, page titles, slide outline, key copy, conclusions, data contract, business numbers, latest month, rankings, recommendations, or per-page chart/layout mapping.
   - **Do not lock arrangement in `spec_lock.md`**: no `page_rhythm`, `page_layouts`, or `page_charts` sections. Layout rhythm, chart choice, and page composition are model-generated from the user request, source data, and `design_spec.md` during SVG authoring.
   - **Allowed default configuration**: high-level layout principles may appear as `layout_policy` (e.g., content-driven, avoid overlap, vary density), but never as `P01/P02/...` page-indexed bindings.
   - `design_spec.md` may contain the model-generated content outline and visualization rationale; `spec_lock.md` must remain a style/technical boundary, not a slide blueprint.
