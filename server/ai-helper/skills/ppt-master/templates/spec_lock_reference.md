...
## layout_policy
- principle: content-driven free layout; no per-page content or chart mapping in spec_lock
- density_rule: choose page density during generation from user request, source data, and design_spec narrative
- template_rule: templates may be referenced in design_spec as model-generated guidance, but spec_lock must not hard-code P01/P02 page bindings
- chart_rule: chart types are selected by the model from data needs, or by scripts from calculated payloads; spec_lock must not assign charts to pages

> Strategist: keep this section high-level. Do NOT add page-indexed rows such as `P01: ...`.
> `spec_lock.md` is an execution boundary for visual tokens and technical constraints only; it is not a slide plan.
